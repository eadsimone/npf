...
class InstapageEdit extends instapage
{
	...
	public function init()
	{
		...
		add_action( 'before_delete_post', array( &$this, 'trashInstapagePost' ) );
	...
	public function trashInstapagePost( $post_id )
	{
		...
		if( $page == null )
		{
			InstapageIO::addNotice( sprintf( __( 'You cannot delete this page (Instapage ID: %s). It was created by another user.' ), $page_id ), 'error' );
			wp_redirect( admin_url( 'edit.php?post_type=instapage_post' ) );
			exit();
		}
		...
		)
		{
			...
			$wp_preview_url = self::endpoint . 'server/view-by-id/' . $page_id;
			...
			catch( InstapageApiCallException $e )
			{
				...
				InstapageIO::addNotice( __( 'There was a problem with changing page status in Instapage application:' ) . ' ' . $e->getMessage() , 'error' );
				wp_redirect( admin_url( 'edit.php?post_type=instapage_post' ) );
				exit();
			}
		}
	...
	public function saveCustomMeta( $post_id, $post )
	{
		...
		try
		{
			...
			InstapageIO::writeDiagnostics( 'updatePageDetails successfull' );
			...
			case '':
				...
				if( $success && $term_id )
				{
					if( $add_notices )
					{
						...
						InstapageIO::addNotice( sprintf( __( 'Selected <strong>Custom URL</strong> (%s) is already in use by WordPress %s. You can <a href="%s">edit the %s</a> and change permalink or change custom Instapage URL.' ), $page_url, $term_name, $wp_term_edit_url, $term_name ), 'error' );
					}
	...
	public function getTermIdByUrl( $url, &$term_name )
	{
		...
		foreach( $terms as $term )
		{
			...
			if( preg_match( $pattern, $instapage_slug, $matches ) )
			{
